...
// deleted; all wire I/O now goes through the typed `Ackstack` resource
...
//   - `createClient` — constructs a `Ackstack` against a target, wired
...
//     `Promise<T>` + thrown `AckstackError` into the same shape the
...
import { Ackstack, AckstackError, type ClientOptions } from "@saas/sdk";
import { apiEdgeWorkersDevUrl } from "./app-config";

export type ApiClient = Ackstack;
...
export function createClient(target: ApiTarget, token: string | null): Ackstack {
  const opts: ClientOptions = { baseUrl: target.url };
  if (token) opts.auth = { kind: "bearer", token };
  return new Ackstack(opts);
}

/**
 * Adapt an SDK promise into the `ApiResult<T>` envelope. Typed
 * `AckstackError` subclasses surface as `{ ok: false }` carrying the
