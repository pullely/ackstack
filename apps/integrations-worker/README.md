...
Your backend holds one Ackstack API key and **zero GitHub credentials**.
...
const ackstack = new Ackstack({
  baseUrl: process.env.ACKSTACK_API_URL!,
  token: process.env.ACKSTACK_API_KEY!, // service-principal key
});

// Mint a token scoped to exactly the repos + permissions you need.
// TTL ≤ 1h; mint per task and let it expire — never store it.
const { token } = await ackstack.integrations.issueGithubToken(ORG_ID, {
