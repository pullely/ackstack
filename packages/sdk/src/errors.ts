// Typed error hierarchy for the Ackstack SDK.
//
...
// codes (forward compatibility) fall back to the generic `AckstackError`
...
export interface AckstackErrorInit {
 ...
 * `quota_exceeded`) decode to this class — `instanceof AckstackError`
 * still matches.
 */
export class AckstackError extends Error {
  ...
  constructor(init: AckstackErrorInit) {
    super(init.envelope.message);
    this.name = "AckstackError";
...
export class BadRequestError extends AckstackError {
  constructor(init: AckstackErrorInit) {
    super(init);
...
export class UnauthenticatedError extends AckstackError {
  constructor(init: AckstackErrorInit) {
    super(init);
...
export class ForbiddenError extends AckstackError {
  constructor(init: AckstackErrorInit) {
    super(init);
...
export class NotFoundError extends AckstackError {
  constructor(init: AckstackErrorInit) {
    super(init);
...
export class ConflictError extends AckstackError {
  constructor(init: AckstackErrorInit) {
    super(init);
...
export class ValidationError extends AckstackError {
...
export class PreconditionFailedError extends AckstackError {
  constructor(init: AckstackErrorInit) {
    super(init);
...
export class UnsupportedError extends AckstackError {
  constructor(init: AckstackErrorInit) {
    super(init);
...
export class InternalError extends AckstackError {
  constructor(init: AckstackErrorInit) {
    super(init);
...
export interface RateLimitErrorInit extends AckstackErrorInit {
...
export class RateLimitError extends AckstackError {
...
/**
 * Decode an HTTP `Response` into the appropriate `AckstackError` subclass.
 *
 * Forward-compatible: unknown error codes resolve to the base `AckstackError`.
...
export async function decodeError(
  ...
  switch (envelope.code as ErrorCode) {
    ...
    default:
      ...
      return new AckstackError(init);
  }
}

function decodeRateLimit(
  init: AckstackErrorInit,
  response: Response,
): RateLimitError {
