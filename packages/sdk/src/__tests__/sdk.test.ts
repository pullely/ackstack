...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new Ackstack({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => AckstackError]> = [
  ...
  it("forward-compat: unknown error code → base AckstackError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(AckstackError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as AckstackError).code).toBe("future_quota_exceeded");
    }
  });
