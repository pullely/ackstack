// Base HTTP transport for the Ackstack SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `AckstackError`
...
export interface ClientOptions {
  /**
   * Base URL of the Ackstack api-edge worker, e.g.
   * `https://api.ackstack.app`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `Ackstack` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "Ackstack SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "Ackstack SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
