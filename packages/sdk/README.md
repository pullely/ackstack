...
Ackstack TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the Ackstack control plane API.

## Install (workspace)

This package is internal to the `ackstack` monorepo. Add it as a
...
```ts
import { Ackstack } from "@saas/sdk";

const client = new Ackstack({
  baseUrl: "https://api.ackstack.app",
  auth: { kind: "bearer", token: process.env["ACKSTACK_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `AckstackError`. Branch on
...
| `AckstackError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `AckstackError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new Ackstack({ fetch })`.
