...
`ackstack` — first-class TypeScript CLI for the Ackstack control
...
This package is internal to the `ackstack` monorepo and is not
...
```
ackstack login    [--api-url=URL] [--token=BEARER]
ackstack logout
ackstack whoami
```
...
```
ackstack org list
ackstack org use <org-id>
ackstack org members
ackstack project list
```

Writes (Task 0101):

```
ackstack org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
ackstack project create <name> [--idempotency-key=KEY]
ackstack env create <project-id> <name> [--idempotency-key=KEY]
ackstack api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
ackstack webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
ackstack usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
ackstack billing summary
ackstack audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`ackstack org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `ackstack login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/ackstack/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/ackstack/config.json` (mode 0644, not a secret). Override
both via `ACKSTACK_CONFIG_DIR` (used by tests).
