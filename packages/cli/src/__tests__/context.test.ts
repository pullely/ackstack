...
describe("resolveConfigDir", () => {
  it("respects ACKSTACK_CONFIG_DIR override", () => {
    const prev = process.env["ACKSTACK_CONFIG_DIR"];
    process.env["ACKSTACK_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["ACKSTACK_CONFIG_DIR"];
      else process.env["ACKSTACK_CONFIG_DIR"] = prev;
    }
  });
});
